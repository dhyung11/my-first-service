# 자격증 트래커 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 보안 자격증 취득 현황·공부 진행도를 사용자별로 관리하는 `/certs` 탭을 구현한다.

**Architecture:** 백엔드에 `certifications`(사전 목록)과 `user_certifications`(사용자별 트래킹) 테이블을 추가하고 FastAPI 라우터 5개 엔드포인트로 CRUD를 제공한다. 프론트엔드는 NewsPage 패턴을 따르는 `CertsPage.jsx`를 신규 생성하고, Header 탭과 App.jsx 라우트를 추가한다. 앱 시작 시 사전 정의 자격증 15개를 자동 시드한다.

**Tech Stack:** Python 3.11 / FastAPI / SQLAlchemy async / PostgreSQL, React 18 / Vite
...
| 파일 | 작업 |
|------|------|
| `backend/models.py` | `Certification`, `UserCertification` 모델 추가 |
| `backend/schemas.py` | `CertificationRead`, `UserCertCreate`, `UserCertUpdate`, `UserCertRead` 추가 |
| `backend/routers/certifications.py` | 신규 — 5개 엔드포인트 + seed 함수 |
| `backend/main.py` | router 등록, lifespan에 seed 호출 추가 |
| `frontend/src/api.js` | API 함수 5개 추가 |
| `frontend/src/pages/CertsPage.jsx` | 신규 — 자격증 트래커 페이지 |
| `frontend/src/components/Header.jsx` | "자격증" 탭 추가 |
| `frontend/src/App.jsx` | `/certs` 라우트 추가 |
...
- [ ] **Step 1: `Integer` import 추가 및 두 모델 append**

`backend/models.py` 상단 import 줄을 아래로 교체한 뒤 파일 끝에 두 클래스를 추가한다.

```python
# import 줄 교체 (Integer 추가)
...
```bash
python -c "from backend.models import Certification, UserCertification; print('OK')"
```
...
git commit -m "feat: Certification, UserCertification 모델 추가"
```

---

## Task 2: Backend 스키마 추가
...
```bash
python -c "from backend.schemas import CertificationRead, UserCertCreate, UserCertUpdate, UserCertRead; print('OK')"
```
...
git commit -m "feat: 자격증 트래커 Pydantic 스키마 추가"
```

---

## Task 3: Backend 라우터 생성

**Files:**
- Create: `backend/routers/certifications.py`
...
```bash
python -c "from backend.routers.certifications import router, seed_certifications; print('OK')"
```
...
```bash
git add backend/routers/certifications.py
git commit -m "feat: 자격증 트래커 API 라우터 추가 (CRUD + seed)"
```

---

## Task 4: main.py — 라우터 등록 + seed 호출
...
`app.include_router(news.router)` 아래에 추가:
```python
...
Expected: 에러 없이 `Application startup complete.` 출력.
`GET http://localhost:8000/api/certifications` 에서 15개 자격증 JSON 반환 확인.

- [ ] **Step 5: Commit**

```bash
git add backend/main.py
git commit -m "feat: certifications 라우터 등록 및 시드 데이터 초기화"
```
...
git commit -m "feat: 자격증 트래커 API 함수 추가"
```

---

## Task 6: CertsPage.jsx 생성

**Files:**
- Create: `frontend/src/pages/CertsPage.jsx`
...
```bash
git add frontend/src/pages/CertsPage.jsx
git commit -m "feat: CertsPage.jsx 자격증 트래커 페이지 추가"
```

---

## Task 7: Header + App.jsx 라우팅 연결

**Files:**
- Modify: `frontend/src/components/Header.jsx`
- Modify: `frontend/src/App.jsx`

- [ ] **Step 1: Header.jsx — `onCerts` 탭 추가**
...
를 `const onNews = ...` 아래에 추가한 뒤, 탭 배열을:
...
- [ ] **Step 2: App.jsx — import + Route 추가**
...
git commit -m "feat: Header에 자격증 탭 추가, /certs 라우트 등록"
```
...
- [ ] 백엔드 기동 후 `GET /api/certifications` → 15개 자격증 JSON 반환
- [ ] 로그인 후 `/certs` 접속 → 빈 상태 화면
- [ ] "자격증 추가" → 사전 목록 선택 → 저장 → 카드 표시
- [ ] 직접 입력 모드로 커스텀 자격증 추가
- [ ] 카드 수정 → 상태/진행도/날짜 변경 → 저장 → 반영 확인
- [ ] 카드 삭제 → 목록에서 제거 확인
- [ ] 사이드바 필터 — 상태/카테고리 필터링 동작 확인
- [ ] 모바일: 필터 드로어, 카드 레이아웃 확인
